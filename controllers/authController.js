...
const login = async (req, res) => {
 ...
 try {
  ...
  const token = jwt.sign({ id: user._id, role: user.role }, process.env.JWT_KEY, { expiresIn: "3d" });

  res.status(200).json({ msg: "Login SuccessFully", token, name: user.name, email: user.email, role: user.role })
